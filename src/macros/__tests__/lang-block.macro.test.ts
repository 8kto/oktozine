...
const getConfig = (buildLang?: string): IDocumentConfig => ({ id: 'main', buildLang }) as unknown as IDocumentConfig
...
describe('parses {% lang %} blocks', () => {
  ...
  it('should handle multiple independent lang-block groups in the same document', () => {
    ...
    expect(parseLangBlocks(input, getConfig('en'))).toEqual('A middle C')
  })

  it('should leave markdown unchanged if there are no lang blocks', () => {
    const input = 'No macros here.'
    expect(parseLangBlocks(input, getConfig('en'))).toEqual(input)
  })
})
